...
const SpiritualStore = () => {
  ...
  // Color definitions with white background
  const colors = {
    white: {
      pure: 'bg-white',
      soft: 'bg-white'
  ...
  return (
    <div className={`relative overflow-hidden py-16 ${colors.white.pure}`}>
      {/* Floating Divine Symbols - More Subtle on White */}
      <motion.div 
        animate={{
          y: [0, -25, 0],
          rotate: [0, 10, -5, 0],
          opacity: [0.05, 0.15, 0.05] // More subtle on white
        }}
        transition={{
          duration: 15,
          repeat: Infinity,
        ...
        className="absolute top-1/4 left-1/4 text-6xl text-amber-300/20"
      >
        ॐ
      </motion.div>
      ...
      <motion.div 
        ...
        className="absolute bottom-1/3 right-1/4 text-7xl text-green-300/20"
      >
        ✸
      </motion.div>
      ...
      <div className="container mx-auto px-4">
        ...
        {/* Sacred Features - Now with white cards */}
        <motion.div 
          initial={{ opacity: 0 }}
        ...
        >
          {sacredFeatures.map((feature, index) => (
            <motion.div
              ...
              className={`${feature.bg} p-6 rounded-xl shadow-lg text-center ${feature.bg.includes('gradient') ? '' : 'border border-amber-100'}`}
            >
              <motion.div
                animate={{
                  scale: hoveredItem === index ? [1, 1.2, 1] : 1,
                  rotate: hoveredItem === index ? [0, 15, -15, 0] : 0
                }}
                transition={{
                  duration: 1.5,
              ...
              <h3 className={`text-xl font-bold ${
                feature.bg.includes('gradient') ? 'text-white' : 'text-amber-700'
              } mb-2`}>
                {feature.title}
              </h3>
              <p className={feature.bg.includes('gradient') ? 'text-amber-100' : 'text-amber-600'}>
                {feature.description}
              </p>
            </motion.div>
          ))}
        ...
        <motion.div 
          ...
          transition={{ duration: 0.8 }}
          className={`${colors.white.pure} p-8 rounded-2xl border border-amber-100 shadow-sm`}
        >
          <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
            <div>
              <motion.h3 
                animate={{
                  textShadow: ["0 0 5px rgba(245, 158, 11, 0.1)", "0 0 10px rgba(245, 158, 11, 0.2)", "0 0 5px rgba(245, 158, 11, 0.1)"]
                }}
                transition={{
                  duration: 3,
                  repeat: Infinity
              ...
              <p className="text-amber-500">Blessed by temple priests</p>
            </div>
            <motion.button
              whileHover={{ 
                scale: 1.05,
              ...
              whileTap={{ scale: 0.95 }}
              className={`px-6 py-3 ${colors.saffron.dark} text-white rounded-lg font-medium shadow-md hover:shadow-amber-500/20 transition-all flex items-center gap-2`}
            >
              View All
              <motion.span
                animate={{
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {newSacredArrivals.map((item, index) => (
              <motion.div
                ...
                className={`${colors.white.pure} p-4 rounded-lg shadow-sm text-center cursor-pointer hover:shadow-md transition-all border border-amber-100`}
              >
                <div className={`w-12 h-12 ${colors.saffron.light} rounded-full flex items-center justify-center mb-3 text-2xl mx-auto`}>
                  {index % 3 === 0 ? '🪔' : index % 3 === 1 ? '📿' : '🕉️'}
                </div>
                <p className="text-sm font-medium text-amber-700">{item}</p>
                <motion.div 
        ...
        >
          <motion.div
            ...
            className="text-6xl mb-4 text-amber-500"
          >
            ॐ
          </motion.div>
          <h3 className="text-2xl font-bold text-amber-600 mb-4 font-serif">Our Divine Promise</h3>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className={`px-8 py-3 ${colors.saffron.dark} text-white rounded-full font-medium shadow-lg transition-all`}
          >
